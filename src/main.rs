...
fn main() {
    ...
    println!("Found {:?} posible files to patch", files.len());
    for x in files {
        let hash = hash_files(&x);
        replace_old_files(&hash, &x);
    }
    println!("All Files Patched Successfully.");
    thread::sleep(time::Duration::from_millis(10000));
}

//
// The stored values in this method has the md5 hashes from all the established core files that need replaced. 
// Hashing is preformed using Blake2b (Blake2 was introduced to replace md5 and SHA-1 after both were considered to be broken)
//
fn replace_old_files(hash: &String, path: &PathBuf) {
    ...
    if x86_files.contains(&hash.as_str()) {
        let p = path.as_path();
    ...
    else if x64_files.contains(&hash.as_str()) {
        let p = path.as_path();
        std::fs::remove_file(&p).expect("Unable to remove a file");
        let mut cracked_file = File::create(&p).expect("Unable to create cracked file, a reinstall will now be required.");
...
//
// Given a file use the Blake algorithim and crypto acceptable algorithim. 
//
...
//
// Scan the entire C: drive and looks for all amtlib.dll files. The actual file that needs patched.
//
